...
import { dealService, DealCategory, DealSubcategory, CreateDealCategoryData, CreateDealSubcategoryData } from '../services/dealService';

const AdminDealManagementPage: React.FC = () => {
  ...
  const [editingSubcategory, setEditingSubcategory] = useState<DealSubcategory | null>(null);

  // Form states
  const [categoryForm, setCategoryForm] = useState<CreateDealCategoryData>({
    name: '',
    icon: '',
    display_order: 0
  });
  const [subcategoryForm, setSubcategoryForm] = useState<CreateDealSubcategoryData>({
    category_id: 0,
    name: '',
    slug: '',
    display_order: 0
  ...
  const loadData = async () => {
    try {
      ...
      const [categoriesData, subcategoriesData] = await Promise.all([
        dealService.getAllDealCategories(),
        dealService.getAllDealSubcategories()
      ]);
  ...
  const handleCategorySubmit = async () => {
    try {
      if (editingCategory) {
        await dealService.updateDealCategory(editingCategory.id, categoryForm);
        setSuccess('Category updated successfully');
      } else {
        await dealService.createDealCategory(categoryForm);
  ...
  const handleSubcategorySubmit = async () => {
    try {
      if (editingSubcategory) {
        await dealService.updateDealSubcategory(editingSubcategory.id, subcategoryForm);
        setSuccess('Subcategory updated successfully');
      } else {
        await dealService.createDealSubcategory(subcategoryForm);
  ...
  const handleDeleteCategory = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this category? All subcategories will also be deleted.')) {
      try {
        await dealService.deleteDealCategory(id);
  ...
  const handleDeleteSubcategory = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this subcategory?')) {
      try {
        await dealService.deleteDealSubcategory(id);
  ...
  const openCategoryDialog = (category?: DealCategory) => {
    if (category) {
  ...
  const openSubcategoryDialog = (subcategory?: DealSubcategory) => {
    if (subcategory) {
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
        Manage deal categories and subcategories for the hamburger menu
      </Typography>

      {/* Categories Section */}
      <Paper sx={{ p: 3, mb: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Typography variant="h6">Deal Categories</Typography>
          <Button
            variant="contained"
            startIcon={<AddIcon />}
      ...
      <Paper sx={{ p: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Typography variant="h6">Deal Subcategories</Typography>
          <Button
            variant="contained"
            startIcon={<AddIcon />}
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12}>
              <TextField
                ...
                helperText="URL-friendly version of the name (e.g., 'limited-time-offers')"
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
...
export default AdminDealManagementPage;
